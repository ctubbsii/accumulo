...
public class ManagerApiIT extends SharedMiniClusterBase {
  ...
  public void testPermissions_initiateFlush() throws Exception {
    ...
    op = user -> client -> client.initiateFlush(null, user, new TTable(tableId, tableName));
    expectPermissionDenied(op, regularUser);
  ...
  public void testPermissions_waitForFlush() throws Exception {
    ...
    TTable thriftTable = new TTable(tableId, tableName);
    op = user -> client -> flushId.set(client.initiateFlush(null, user, thriftTable));
    expectPermissionSuccess(op, rootUser);
    op = user -> client -> client.waitForFlush(null, user, thriftTable,
