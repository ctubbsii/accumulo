...
public enum Property {
  ...
  GENERAL_SERVER_LOCK_VERIFICATION_INTERVAL("general.server.lock.verification.interval", "2m",
      PropertyType.TIMEDURATION,
      "Interval at which the Manager and TabletServer should verify their server locks. A value of zero disables this check. The default value changed from 0 to 2m in 4.0.0.",
      "2.1.4"),
  ...
  SSERV_SCAN_EXECUTORS_PREFIX("sserver.scan.executors.", null, PropertyType.PREFIX, """
      ...
      `sserver.scan.executors.<name>.prioritizer.opts.<key>=<value>`.""", "2.1.0"),
  ...
  SSERV_SCAN_EXECUTORS_DEFAULT_PRIORITIZER("sserver.scan.executors.default.prioritizer", "",
      PropertyType.STRING, """
          ...
          `%s` to configure one.""".formatted(ScanPrioritizer.class.getName()), "2.1.0"),
  ...
  TSERV_MAXMEM("tserver.memory.maps.max", "33%", PropertyType.MEMORY, """
      ...
      * `tserver.wal.max.size` >= this property. This map is created in off-heap
      memory when %s is enabled.""".formatted(TSERV_NATIVEMAP_ENABLED.name()), "1.3.5"),
  ...
  TSERV_SCAN_EXECUTORS_PREFIX("tserver.scan.executors.", null, PropertyType.PREFIX, """
      ...
      `tserver.scan.executors.<name>.prioritizer.opts.<key>=<value>`.""", "2.0.0"),
  ...
  TSERV_SCAN_EXECUTORS_DEFAULT_PRIORITIZER("tserver.scan.executors.default.prioritizer", "",
      PropertyType.STRING, """
          Prioritizer for the default scan executor. Defaults to none which
          results in FIFO priority. Set to a class that implements
          %s to configure one.""".formatted(ScanPrioritizer.class.getName()), "2.0.0"),
  ...
  TABLE_MINC_COMPACT_MAXAGE("table.compaction.minor.age", "10m", PropertyType.TIMEDURATION,
      """
          ...
          scan server to see a write to a tablet server. The default value of this property is set
  ...
  TABLE_FILE_MAX("table.file.max", "15", PropertyType.COUNT, """
      ...
      from having more RFiles than can be opened by a scan.""", "1.4.0"),
  TABLE_FILE_PAUSE("table.file.pause", "100", PropertyType.COUNT,
      """
          ...
          .formatted(TABLE_BULK_MAX_TABLET_FILES.getKey(), TABLE_FILE_MAX.getKey()),
      "4.0.0"),
  ...
  TABLE_BLOOM_KEY_FUNCTOR("table.bloom.key.functor",
      "org.apache.accumulo.core.file.keyfunctor.RowFunctor", PropertyType.CLASSNAME, """
          ...
          perform specialized parsing of the key.""", "1.3.5"),
  ...
  TABLE_DURABILITY("table.durability", "sync", PropertyType.DURABILITY, """
      ...
      data to the file system; and sync, which ensures the data is written to disk.""", "1.7.0"),
  ...
  TABLE_ITERATOR_PREFIX("table.iterator.", null, PropertyType.PREFIX, """
      ...
      For example:
        table.iterator.minc.vers.opt.maxVersions = 3.""", "1.3.5"),
  ...
  TABLE_LOCALITY_GROUP_PREFIX("table.group.", null, PropertyType.PREFIX, """
      ...
      `table.group.<name>.opt.<key>=<value>`.""".formatted(TABLE_LOCALITY_GROUPS.getKey()),
      "1.3.5"),
  ...
  TABLE_DELETE_BEHAVIOR("table.delete.behavior",
      ...
      """
          ...
          .formatted(NoDeleteConstraint.class.getName()),
      "2.0.0"),
  TABLE_ENABLE_ERASURE_CODES("table.file.ec", "inherit", PropertyType.EC, """
      ...
      "that does not support it is a noop.""", "2.1.4"),
  ...
  COMPACTOR_FAILURE_BACKOFF_INTERVAL("compactor.failure.backoff.interval", "0",
      PropertyType.TIMEDURATION, """
          ...
          `compactor.failure.backoff.threshold` failures have occurred, the compactor will wait
          `compactor.failure.backoff.interval * the number of failures` seconds before executing
          the next compaction.
          ...
          wait `40s` before starting the next compaction.""", "2.1.4"),
  ...
  COMPACTION_COORDINATOR_PREFIX("compaction.coordinator.", null, PropertyType.PREFIX,
      "Properties in this category affect the behavior of the accumulo compaction coordinator services in the manager.",
      "2.1.0"),
