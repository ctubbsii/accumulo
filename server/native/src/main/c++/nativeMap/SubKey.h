...
public:
  ...
  int compare(const uint8_t *d1, int len1, const uint8_t *d2,
      int len2) const {
   ...
   */
  SubKey(LinkedBlockAllocator *lba, const string &cf, const string &cq,
      const string &cv, int64_t ts, bool del, int32_t mc) {
  ...
  SubKey(LinkedBlockAllocator *lba, JNIEnv *env, jbyteArray cf, jbyteArray cq,
      jbyteArray cv, jlong ts, jboolean del, int32_t mc) {
    ...
    env->GetByteArrayRegion(cq, 0, cqLen,
        (jbyte*) (keyData + colQualifierOffset));
    env->GetByteArrayRegion(cv, 0, cvLen,
        (jbyte*) (keyData + colVisibilityOffset));
  ...
  bool operator<(const SubKey &key) const {
    ...
    int result = compare(keyData, colQualifierOffset, key.keyData,
        key.colQualifierOffset);
    if (result != 0)
      return result < 0;

    result = compare(keyData + colQualifierOffset,
        colVisibilityOffset - colQualifierOffset,
        key.keyData + key.colQualifierOffset,
        key.colVisibilityOffset - key.colQualifierOffset);
    if (result != 0)
      return result < 0;

    result = compare(keyData + colVisibilityOffset,
        totalLen - colVisibilityOffset,
        key.keyData + key.colVisibilityOffset,
        key.totalLen - key.colVisibilityOffset);
    if (result != 0)
      return result < 0;
  ...
  string toString() const {
    return getCF().toString() + ":" + getCQ().toString() + ":"
        + getCV().toString();
  }
...
struct LocalSubKey: public SubKey {
  ...
  LocalSubKey(JNIEnv *env, jbyteArray cf, jbyteArray cq, jbyteArray cv,
      jlong ts, jboolean del) : SubKey(NULL, env, cf, cq, cv, ts, del,
      INT_MAX) {
  }
