...
public:
  ...
  Key(const string &r, const string &cf, const string &cq, const string &cv,
      long ts, bool del) {
  ...
  Key(JNIEnv *env, jbyteArray kd, jint cfo, jint cqo, jint cvo, jint tl,
      jlong ts, jboolean del) {
  ...
  bool operator<(const Key &key) const {
    int result = compare(keyData, colFamilyOffset, key.keyData,
        key.colFamilyOffset);
    if (result != 0)
      return result < 0;

    result = compare(keyData + colFamilyOffset,
        colQualifierOffset - colFamilyOffset,
        key.keyData + key.colFamilyOffset,
...
public:
  ...
  LocalKey(JNIEnv *env, jbyteArray kd, jint cfo, jint cqo, jint cvo, jint tl,
      jlong ts, jboolean del) {
    envPtr = env;
