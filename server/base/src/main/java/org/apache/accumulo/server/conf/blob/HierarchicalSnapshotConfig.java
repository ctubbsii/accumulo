...
public class HierarchicalSnapshotConfig implements SnapshotConfig {

  private static final Logger log = LoggerFactory.getLogger(SnapshotConfig.class);

  private final Map<String,String> config;
  private final SnapshotConfig parent;
  public final int sequenceId;

  public HierarchicalSnapshotConfig(int seqId, final Map<String,String> config, final Optional<SnapshotConfig> parent) {
    this.sequenceId = seqId;
    this.config = Collections.unmodifiableMap(config);
    this.parent = parent.orElse(SnapshotConfig.EMPTY);
  }
  ...
  public String toJSON() {
    return new VersionedConfBlob(config).serialize();
  }
